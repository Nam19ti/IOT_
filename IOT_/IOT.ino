...
const int TRIGGER_CAM_PIN = 27;   // GPIO 27 - Kích hoạt ESP32-CAM chụp ảnh biển số xe
...
void calculateSpeed(unsigned long t_start, unsigned long t_end, String direction) {
  ...
  static unsigned long car_id = 0;
  car_id++;

  // Gui toc do + chieu qua UART cho ESP32 thu 2
  // Dinh dang: "SPEED:25.3,DIR:Trai->Phai,ID:1\n"
  Serial2.print("SPEED:");
  Serial2.print(speed_kmph, 1);
  Serial2.print(",DIR:");
  Serial2.print(direction); 
  Serial2.print(",ID:");
  Serial2.println(car_id); // println tu them '\n'
  Serial.println(">> UART: Da gui SPEED=" + String(speed_kmph,1) + " DIR=" + direction + " ID=" + String(car_id));
...
void loop() {
  ...
  if (!isArmed) {
    if (millis() - lastSerialPrint >= 500) {
      ...
      // --- In trạng thái ra Serial Monitor và gửi qua UART mỗi 5 giây ---
