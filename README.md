# DỰ ÁN: HỆ THỐNG TRẠM THU PHÍ TỰ ĐỘNG
...
Giao diện quản lý (Web UI) được xây dựng dựa trên triết lý **Kiosk-Mode** (Tối giản, trực quan, chống thao tác nhầm), phục vụ trực tiếp cho nhân viên bảo vệ tại bốt:
+ **Công nghệ Backend & Routing:** Dựng trên nền tảng Micro-framework `Flask` của Python. Các luồng dữ liệu nội bộ được chia thành các Endpoint API RESTful riêng biệt (VD: `/api/history`, `/api/trigger_manual`).
+ **Công nghệ Frontend (Giám sát thời gian thực):** Sử dụng HTML5/CSS3 kết hợp JavaScript Ajax. Giao diện có khả năng tự động gửi truy vấn ngầm (Polling) tới Server để làm mới danh sách xe vào ra mà không cần phải tải lại (Reload) toàn bộ trang web, mang lại trải nghiệm mượt mà như một ứng dụng Native.
+ **Bảng Điều khiển Khẩn cấp (Override Controls):** Giao diện cung cấp 3 phím bấm phần mềm (Soft-keys) liên kết thẳng tới mạch ESP32:
  - `MỞ CỔNG (Không Tự Đóng)`: Dùng cho trường hợp xe cứu hỏa, cứu thương cần giữ cổng mở liên tục.
  - `MỞ CỔNG (Tự Động Đóng)`: Dùng khi AI bị chói sáng/bùn đất không đọc được biển, bảo vệ kiểm tra bằng mắt và bấm mở. Cổng vẫn tuân thủ thuật toán siêu âm để tự hạ xuống sau khi xe qua.
  - `ĐÓNG CỔNG KHẨN CẤP`: Ép Barie hạ xuống ngay lập tức (với điều kiện cảm biến xác nhận không có xe nằm dưới cổng).
+ **Quản lý Lịch sử Đối chứng:** Bảng dữ liệu trích xuất từ MongoDB được phân trang (Pagination) tự động. Mỗi bản ghi (Record) đều đính kèm một hình ảnh cắt ra từ thẻ nhớ (Snapshot) tại đúng sát mili-giây xe chạm vạch. Tính năng này đóng vai trò là "Hộp đen" để đối chiếu nếu xảy ra tranh chấp vé xe.

### 6.2. Cơ chế lấy mẫu ảnh và Nhận diện OCR đám mây
Khâu thắt cổ chai lớn nhất của các dự án Camera IP là độ trễ truyền hình ảnh. Hệ thống đã giải quyết triệt để vấn đề này bằng các kỹ thuật tối ưu hóa luồng:
+ **Giao thức lấy mẫu tĩnh Siêu tốc (Zero-Overhead Snapshot):** Thay vì sử dụng giao thức RTSP nặng nề hay tải luồng Live Stream liên tục làm nghẽn băng thông mạng LAN, dự án áp dụng phương pháp Bắt ảnh Tĩnh (Static Fetch). Cụ thể, Python Server sử dụng thư viện mạng cấp thấp `urllib.request` để gửi gói HTTP GET không Header thẳng vào đường dẫn `/shot.jpg` của IP Webcam. Camera sẽ nhả ngay 1 khung ảnh JPEG thô với độ trễ < 0.1s.
+ **Giải mã Ma trận ảnh trên RAM:** Luồng byte tải về không được lưu tạm ra ổ cứng (tránh hại ổ cứng và làm chậm hệ thống). Thay vào đó, nó được đưa thẳng lên RAM và giải nén bằng kỹ thuật `cv2.imdecode(np.frombuffer(raw_bytes, np.uint8), -1)` của thư viện OpenCV để tạo thành ma trận điểm ảnh RGB, sẵn sàng cho việc cắt xén (Crop) bỏ viền thừa, giảm kích thước ảnh trước khi đẩy lên Internet.
+ **Phân tích OCR đám mây bằng LLM & Regex:** Ảnh đã xử lý được mã hóa Base64 và đính kèm vào yêu cầu gửi lên Google Gemini Vision API. Để ép AI (Vốn được sinh ra để giao tiếp dài dòng) phải làm việc như một cỗ máy đọc OCR thuần túy, Lời nhắc (Prompt) được thiết kế cực kỳ khắt khe: *"Chỉ trả về JSON định dạng `{"plate": "xxx"}`. Cấm mọi bình luận"*. Cuối cùng, hàm `re.search(r'\{.*\}')` (Regular Expression) trên Python sẽ làm nhiệm vụ quét lại chuỗi phản hồi, "cạo" sạch các văn bản rác xung quanh để giữ lại đúng khối dữ liệu JSON tinh khiết.

### 6.3. Cơ chế chịu lỗi khi Mất kết nối (Offline Fault Tolerance)
Để đảm bảo trạm thu phí không bao giờ bị "tê liệt" vì lỗi mạng, hệ thống áp dụng tiêu chuẩn Fail-Safe 3 lớp:
+ **Lớp 1 - Chống đứt kết nối nội bộ (Auto-Reconnect):** Mạch ESP32 Gateway (Mạch 2) được lập trình cơ chế Máy trạng thái (State Machine). Nó sử dụng hàm `millis()` để tạo bộ đếm thời gian ảo (Non-blocking). Nếu phát hiện rớt WiFi, mạch không hề bị treo (Freeze) mà sẽ hiển thị chữ `MAT MANG WIFI` lên LCD, đồng thời ngầm chạy tiến trình `WiFi.begin()` mỗi 5 giây cho đến khi kết nối được khôi phục.
+ **Lớp 2 - Chống sập Máy chủ (Exception Isolation):** Nếu đường truyền LAN ổn định nhưng Server Python bị sập (do mất điện) hoặc Camera hết pin, thư viện `HTTPClient` trên ESP32 sẽ bị Timeout thay vì chờ đợi vô tận. Ở chiều ngược lại, mọi hàm gọi mạng trên Python đều được bọc trong khối `try...except`. Nhờ vậy, lỗi kết nối chỉ hiển thị lên màn hình log chứ không bao giờ làm Crash toàn bộ chương trình phần mềm.
+ **Lớp 3 - Độc lập Phần cứng (Hardware Decentralization):** Đây là điểm sáng giá nhất của mô hình 2 mạch ESP32 phân tán. Giả sử toàn bộ mạng WiFi, Máy chủ Python và Camera đồng loạt bốc cháy, **Mạch 1 (Cơ điện & Master Logic) vẫn "sống sót" 100%**. Bảo vệ vẫn có thể dùng nút bấm cơ học để mở cổng. Thuật toán phân tích sóng âm đa luồng (quét 3 lần liên tiếp để loại bỏ nhiễu phản xạ của kính ô tô) và tính năng Zero-Crash chống đập barie vào xe vẫn vận hành hoàn hảo, biến trạm đỗ xe thông minh thành một bốt đỗ xe truyền thống dùng nút bấm trong những tình huống khẩn cấp nhất.
...
## 7. Cơ chế Thu Phí Tự Động (VETC)
Hệ thống sử dụng MongoDB để quản lý tài khoản và số dư (`balance`) của từng xe. 
- Khi xe đi qua, AI nhận diện biển số và truy vấn vào MongoDB.
